...
const MessageMenu = ({ message }) => {
  ...
  const [copyIcon, setCopyIcon] = useState(<MdContentCopy size={17} className='text-main hover:bg-main hover:text-slate-300 hover:p-1 rounded' />);
  ...
  const menuItems = [
      ...
      action: () => {
        navigator.clipboard.writeText(message)
          .then(() => {
            ...
            setTimeout(() => setCopyIcon(<MdContentCopy size={17} className='text-main hover:bg-main hover:text-slate-300 hover:p-1 rounded' />), 3000);
          })
          .catch((err) => console.error('Failed to copy: ', err));
      }
    },
    {
      icon: <MdRefresh size={20} className='text-main hover:bg-main hover:text-slate-300 hover:p-1 rounded' />,
      title: 'Refresh',
      action: () => {
    ...
    {
      icon: <MdSaveAlt size={20} className='text-main hover:bg-main hover:text-slate-300 hover:p-1 rounded' />,
    ...
    {
      icon: isSpeaking ? <MdStop size={20} className='text-main hover:bg-main hover:text-slate-300 hover:p-1 rounded' /> : <MdPlayArrow size={20} className='text-main hover:bg-main hover:text-slate-300 hover:p-1 rounded' />,
    ...
    <div className='flex space-x-4'>
      {menuItems.map((item, index) => (
        ...
        >
          {item.icon}
          {hoveredIndex === index && (
            <span className='absolute top-4 left-1/2 transform -translate-x-1/2 bg-gray-800 text-slate-300 text-xs rounded p-1 mt-2'>
              {item.title}
            </span>
          )}
        </div>
